...
class LightGCL(nn.Module):
    ...
    def forward(self, uids, iids, pos, neg, test=False):
        if test:
            ...
            mask = torch.tensor(mask, device=preds.device, dtype=preds.dtype)
        ...
        # training: propagate & compute losses
        for layer in range(1, self.l + 1):
            self.Z_u_list[layer] = torch.spmm(sparse_dropout(self.adj_norm, self.dropout),
        ...
        pos_score = (
            torch.clamp((G_u_norm[uids] * E_u_norm[uids]).sum(1) / self.temp, -5.0, 5.0).mean()
